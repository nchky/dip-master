...
//nch 空域滤波对话框（包含：roberts、sobel、prewitt、laplacian4、laplacian8、LOG）
...
//nch 根据现有代码进行的更改
SDFilterDialog::SDFilterDialog(QImage inputImage,int sf)
{
    srcImage = inputImage;
...
void SDFilterDialog::iniUI(int sf)
{
    // four image labels
    srcImageLabel = new QLabel();
    srcImageLabel->setAlignment(Qt::AlignCenter);
    ...
    QString sfName;
    if(sf==0)sfName=tr("Roberts");
    if(sf==1)sfName=tr("Sobel");
    if(sf==2)sfName=tr("Prewitt");
    if(sf==3)sfName=tr("Laplacian4");
    if(sf==4)sfName=tr("Laplacian8");

    robertsLabel = new QLabel(sfName);
    robertsLabel->setAlignment(Qt::AlignCenter);
