...
//nch RGB图像的通道分离，输入Image输出的是r、g、b三个浮点型数组
...
//nch RGB图像的通道分离，输入Image输出的是rgb一个浮点型数组
...
//nch RGB图像的通道分离，输入Image输出的是r、g、b三个字符型数组
...
//nch RGB图像的通道分离，输入Image输出的是rgb一个字符型数组
...
//nch 图像的通道合并，输入r、g、b三个浮点型数组及w、h图像的宽和高输出的是image图像
...
//nch 图像的通道合并，输入rgb一个浮点型数组及w、h图像的宽和高输出的是image图像
...
//nch 图像的通道合并，输入r、g、b三个字符型数组及w、h图像的宽和高输出的是image图像
...
//nch 图像的通道合并，输入rgb一个字符型数组及w、h图像的宽和高输出的是image图像
...
// nch RGBImage to ycrcbImage，Y:brightness cr:chroma cb:saturation
//Input:
//Output:
...
///////////////////////////////////////////////////////////////////////////////
//功能：计算图像直方图
//输入:image为要处理的图像  channel为要处理的通道
//输出:为直方图的图像
//nch
////////////////////////////////////////////////////////////////////////////
QImage calculateHistogram(QImage &image, ImageChannel channel)
{
    ...
    float s_h = 0.8;           //nch 取0.8？

    for (int i=0; i<gray_level; i++)
    {
        int v = hist[i];
        hist[i] = int(h*1.0/max_hist_val * s_h * v);   //nch 将hist的值转换到（0-0.8）*512
    }
...
QImage equalizeHistogramProc1(QImage &image)
{
    ...
    rgb2ycrcb(r, g, b, pixel_num, y, cr, cb);  //nch 只对亮度做直方图均衡，否则容易出现色偏
...
void calculate_integral_image_power(float *image, int width, int height, float *integral_image)
{
    ...
    for(i=1; i<height; ++i)
    {
        ...
        for(j=0; j<width; ++j)
        {
            rs += image[i*width+j]*image[i*width+j];   //nch 平方和
            integral_image[i*width+j] = rs + integral_image[(i-1)*width+j];
        }
    }
}
